...
func TestJsonNumIncrCommand(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "OK", res)
	cmd1 := redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.l1.l2_a", 1.6350000000001313e+308)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err := cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[null]", str)
	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMMULTBY", key, "$.l1.l2_a", 1.6350000000001313e+308)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err = cmd1.Result()
	assert.NoError(t, err)
	...
	res, err = rh.JSONSet(key, ".", map[string]interface{}{
		"a": []interface{}{}, "b": []int{1},
		"c": []int{1, 2}, "d": []int{1, 2, 3},
	})
	assert.NoError(t, err)
	assert.Equal(t, "OK", res)
	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.d[*]", 10)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[11,12,13]", str)
	resBytes, err = Bytes(rh.JSONGet(key, "."))
	assert.NoError(t, err)
	assert.JSONEq(t, "{\"a\":[],\"b\":[1],\"c\":[1,2],\"d\":[11,12,13]}", string(resBytes))

	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.a[*]", 1)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[]", str)
	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.b[*]", 1)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[2]", str)
	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.c[*]", 1)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[2,3]", str)
	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.d[*]", 1)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[12,13,14]", str)
	resBytes, err = Bytes(rh.JSONGet(key, "."))
	assert.NoError(t, err)
	assert.JSONEq(t, "{\"a\":[],\"b\":[2],\"c\":[2,3],\"d\":[12,13,14]}", string(resBytes))
	...
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[2,3,4]", str)
	resBytes, err = Bytes(rh.JSONGet(key, "."))
	assert.NoError(t, err)
	assert.JSONEq(t, "{\"a\":{},\"b\":{\"a\":2},\"c\":{\"a\":2,\"b\":3},\"d\":{\"a\":2,\"b\":3,\"c\":4}}", string(resBytes))

	res, err = rh.JSONSet(key, ".", map[string]interface{}{
		"a": map[string]string{"a": "a"},
		"b": map[string]interface{}{"a": "a", "b": 1},
		"c": map[string]string{"a": "a", "b": "b"},
		"d": map[string]interface{}{"a": 1, "b": "b", "c": 3},
	})
	assert.NoError(t, err)
	assert.Equal(t, "OK", res)
	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.a.*", 1)
	...
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[null,2]", str)
	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.c.*", 1)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[null,null]", str)
	cmd1 = redis.NewStringCmd(ctx, "JSON.NUMINCRBY", key, "$.d.*", 1)
	err = c.Process(ctx, cmd1)
	assert.NoError(t, err)
	str, err = cmd1.Result()
	assert.NoError(t, err)
	assert.JSONEq(t, "[2,null,4]", str)
	resBytes, err = Bytes(rh.JSONGet(key, "."))
	assert.NoError(t, err)
	assert.JSONEq(t, "{\"a\":{\"a\":\"a\"},\"b\":{\"a\":\"a\",\"b\":2},\"c\":{\"a\":\"a\",\"b\":\"b\"},\"d\":{\"a\":2,\"b\":\"b\",\"c\":4}}", string(resBytes))
